...
init();
$("#clearOrders").onclick=async()=>{if(!confirm("Hapus SEMUA pesanan? Pesanan yang sudah dihapus tidak bisa dikembalikan."))return;if(!confirm("Yakin? Produk, akun, dan QR DANA tidak akan dihapus."))return;const r=await fetch("/api/admin/orders/clear-all",{method:"DELETE"}),d=await r.json();if(!r.ok)return alert(d.error||"Gagal menghapus pesanan.");alert(d.message||"Semua pesanan berhasil dihapus.");load()};