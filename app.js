...
function add(e,id){e.preventDefault();e.stopPropagation();let c=JSON.parse(localStorage.cart||'[]'),x=c.find(a=>a.id===id),p=products.find(a=>a.id===id);if(x){if(x.qty>=p.stock)return toast('Jumlah sudah mencapai stok.');x.qty++}else c.push({id,qty:1});localStorage.cart=JSON.stringify(c);updateCartCount();toast('Ditambahkan ke keranjang.');}
